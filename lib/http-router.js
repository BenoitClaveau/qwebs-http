...
class HttpRouter {
    ...
    async invoke(context, ask, reply, headers) {
        try {
            ...
            context.methodName = leaf.router.methodName;
            context.serviceName = leaf.router.serviceName;
            context.options = leaf.router.options;
            context.route = leaf.router.route;

            await leaf.router.invoke(context, ask, reply, headers);
        }
        catch(error) {
            error.url = ask.url;
