...
class HttpRouter {
    ...
    async invoke2(stream, headers, flags) {
        try {
            ...
            //If stream is encrypted -> use a stream wrapper like ask
            
            const part = url.parse(decodeURI(path));
            stream.pathname = part.pathname;
            stream.querystring = part.query;
            if (part.query) stream.query = this.querystring.parse(part.query);
            
            let leaf = await this.leaf(method, stream.pathname);
            if (!leaf) 
                throw new HttpError(404);
            
            stream.params = leaf.params;
            stream.methodName = leaf.router.methodName;
            stream.serviceName = leaf.router.serviceName;
            stream.options = leaf.router.options;
            stream.route = leaf.router.route;

            await leaf.router.invoke2(stream, headers, flags);
        }
        catch(error) {
            stream.respond({
