...
class HttpServer {
	...
	writeError(context, stream, headers, error) {
		const { url, method } = context;
		const { message, stack, data } = error;	
		stream.mode("object");
		stream.statusCode = error.statusCode || 500;
		stream.end({ method, url, message, stack, data });
	}
    ...
    async invoke(request, response) {
		try {
			...
			try {
				...
				if (!response.headersSent) this.writeError(context, stream, headers, error);
				const { url, method } = context;
				const { message, stack, data } = error;				
