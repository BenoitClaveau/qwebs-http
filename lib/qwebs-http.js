...
class HttpServer {
    ...
    async invoke(request, response) {
		try {
			...
			return await this.router.invoke(ask, reply);
		}
		catch(error) {
	...
	async createHttp2Server(config) {
		...
		if (secure) { //openssl req -x509 -newkey rsa:2048 -nodes -sha256 -subj '/CN=localhost' -keyout private-key.pem -out certificate.pem
			let options = {
				key: fs.readFileSync(config.key),
				cert: fs.readFileSync(config.cert),
			};
