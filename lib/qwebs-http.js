...
class HttpServer {
  	constructor($injector, $config, $event) {
		...
		$injector.inject("$reply", { local: true });
    ...
    async invoke(request, response, overriddenUrl) {
		try {
			...
			let reply = await this.$injector.resolve("$reply");
			reply.request = request;
			reply.response = response;

			return await this.router.invoke(reply);
		}
		catch(error) {
            //if (!(error instanceof HttpError)) throw new HttpError(500, "Error on ${method} ${url}", { url: request.url, method: request.method}, error);
			//throw error;
			console.error(error)
        };
	};
