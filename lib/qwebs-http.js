...
class HttpServer {
  	constructor($injector, $config, $event) {
		...
		$injector.inject("$router-http", `${__dirname}/router-http`);
		$injector.inject("$assets-loader", `${__dirname}/loaders/assets`);
		$injector.inject("$routes-http", `${__dirname}/loaders/routes`);
