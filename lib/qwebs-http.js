...
class HttpServer {
    ...
    async invoke(request, response) {
		try {
			...
			const ask = await factory.getAsk(request);
			const reply = await factory.getReply(request);
			const context = await factory.getContext(request);
			const headers = await factory.getHeader(request);

			return await this.router.invoke(context, ask, reply, headers);
		}
		catch(error) {
