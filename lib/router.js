...
class Router {
    ...
    async invoke(request, response) {
        ...
        objectMode = leaf && /false/ig.test(leaf.router.options.objectMode) ? false : true;
        const context = new Context(request, response, this.qwebs, { objectMode });
        try {
            if (!leaf) throw new HttpError(404);
            context.params = leaf.params;
            context.outputType = leaf.router.options.outputType;
            leaf.router.options.contentType && (this.headers["Content-Type"] = leaf.router.options.contentType);
