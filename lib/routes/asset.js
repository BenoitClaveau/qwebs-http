...
class Asset {
    ...
    initFromFiles(filepathList) {
        return Promise.resolve().then(() => {
            if (!filepathList) throw new Error("filepathList is empty.", { filepathList: filepathList, route: this.route });
            if (!Array.isArray(filepathList)) throw new Error("filepathList must be an array.", { filepathList: filepathList, route: this.route });

            for(let i = 0; i < filepathList.length; i++)
            {
                let filepath = filepathList[i] = path.resolve(this.$qwebs.root, filepathList[i]);
                if (!fs.existsSync(filepath)) throw new Error("Asset ${file} doesn't exist.", { file: filepath, route: this.route });
            }
    ...
    _init(content) {
        return Promise.resolve().then(() => {
            if (!content) throw new Error("Content of ${route} is empty.", { route: this.route });
    ...
    _getCssBundle(filepathList) {
        return this._getBundle(filepathList, { ext: ".css", encoding: "utf8" }).then(css => {
            return this._getBundle(filepathList, { ext: ".scss", encoding: "utf8" }).then(scss => {
                if (!css && !scss) throw new Error("Css bundle is empty for ${route}.", { route: this.route });
                if (!scss) return css;
                
                throw new Error("Sass has been desactivated due to bug.")
                
                return new Promise((resolve, reject) => {
                    Sass.compile(scss, result => {
                        if (result.status) reject(new Error("Scss compilation failed.", { result }));
    ...
    invoke(request, response) {
        return Promise.resolve().then(() => {
            ...
            if (!this.content) throw new Error("Content is empty for ${route}.", { route: this.route });
            if (!this.contentType) throw new Error("ContentType is undefined for ${route}.", { route: this.route });
            if (!this.contentGzip) throw new Error("contentGzip is undefined for ${route}.", { route: this.route });
            if (!this.contentDeflate) throw new Error("contentDeflate is undefined for ${route}.", { route: this.route });
