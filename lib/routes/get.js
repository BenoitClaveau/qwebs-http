...
class Get {
    ...
    async invoke (context, stream, headers, flags) {
        const { options, qwebs, route: { options: { auth, forward }, method, service }} = context;
        if (auth) {
            const auth = await qwebs.resolve("$auth");
            await auth.authenticate(context, stream, headers);
        }
        if (forward) return await stream.forward(forward); 
        return await method.call(service, context, stream, headers, flags);
    };
};
