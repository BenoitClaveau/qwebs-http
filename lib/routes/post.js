...
class Post extends Get {
    ...
    parseBody(reply) {
        return Promise.resolve().then(() => {
            if (!this.method) throw new Error("Method is not defined for ${service}.", { service: this.serviceName });
            if (!request) throw new Error("Request is not defined for ${service}.${method}.", { service: this.serviceName, method: this.method });
            if (!response) throw new Error("response is not defined for ${service}.${$method}.", { service: this.serviceName, method: this.method });
            ...
            if (mime == "multipart/form-data") {
                ...
                return new Promise((resolve, reject) => {
                    form.parse(request, (err, fields, files) => {
                        if (err) reject(new HttpError(500, error));
                        else {
                            request.body = {};
    ...
    async invoke(reply) {
        return this.parseBody(reply).then(async () => {
            return await super.invoke(reply);
        });
    };
};
