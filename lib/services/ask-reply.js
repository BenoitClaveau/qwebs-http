...
class AskReply extends Readable {
    ...
    async onceReadableInit() {
        ...
        this[kReadableStreamEnd] = this[kReadableStreamCurrent] = new PassThrough({ objectMode });
    ...
    /* object | aray | buffer */
    mode(m) {
        return this.modeIn(m).modeOut(m);
    }

    /* object | aray | buffer */
    modeIn(m) {
        if (![m].some(e => e == l)) throw new Error("Mode value is not valid. Mode must be 'object', | 'array' | 'buffer'.");
        this[kReadableMode] = m || "array";
        return this;
    }
