...
class AskReply extends Readable {
    ...
    constructor($qwebs, request, response, headers) {
        ...
        this.once('writableinit', () => this.onceWritableInit());
    }
    ...
    async onceReadableInit() {
        ...
        this[kReadableStreamEnd].once('end', () =>  this.onceReadableEnd());

        this[kReadableInit] = true;
        this.emit("readableinit");
    }

    async writableInit(chunk) {
        ...
        //this[kResponse].on('data', (chunk) => this.emit(chunk));

        this[kResponse].writeHead(200, { headers });
        this[kWritableStreamCurrent].pipe(this[kResponse]).on("data", data => {
    ...
    onceReadableEnd() {
        ...
        this[kWritableStreamStart].end();
    }

    read() {
        if (!this[kReadableInit]) {
            this.once('readableinit', this.read.bind(this));
            return;
        }
    ...
    write(chunk, encoding, callback) {
        this[kWritableStreamStart].write(chunk);
    }
