...
class AskReply extends Readable {
    ...
    constructor($qwebs, request, response, headers) {
        ...
        this[kWritableStreamStart] = new PassThrough();
    ...
    async onceReadableInit() {
        ...
        this[kReadableStreamEnd].on('data', data => {
            console.log("[data][Readable End]", data)
            this.emit("data", data ? data : null);
        });
        this[kReadableStreamEnd].once('end', data =>  {
            console.log("[end][Readable End]", data)
            this.emit("end", data);
        });
        this[kReadableStreamEnd].once('error', data =>  {
            console.log("[error][Readable End]", data)
            this.emit("error", data);
        });
        this[kReadableStreamEnd].once('readable', data =>  {
            console.log("[readable][Readable End]", data)
    ...
    async onceWritableInit(chunk, encoding) {
        ...
        this[kWritableStreamCurrent].on("close", data => {
            console.log("[close][Writable Current]", data)
        })
        this[kWritableStreamCurrent].on("drain", data => {
            console.log("[drain][Writable Current]", data)
        })
        this[kWritableStreamCurrent].on("error", data => {
            console.log("[error][Writable Current]", data)
        })
        this[kWritableStreamCurrent].on("finish", data => {
            console.log("[finish][Writable Current]", data)
        })
        this[kWritableStreamCurrent].on("pipe", data => {
            console.log("[pipe][Writable Current]", data)
        })
        this[kWritableStreamCurrent].on("unpipe", data => {
            console.log("[unpipe][Writable Current]", data)
        })
        ...
        this[kWritableStreamCurrent].on("drain", data => {
            console.log("[drain][Response]", data)
        })
        this[kWritableStreamCurrent].on("error", data => {
        ...
        this[kWritableStreamCurrent].on("unpipe", data => {
            console.log("[unpipe][Response]", data)
        })
