...
class AskReply extends PassThrough {
    ...
    async onError(error) {
        ...
        if(!this._readableState.ending && !this._readableState.ended && !this._readableState.destroyed)
            this.write(error);
    ...
    destroy(error) {
        if (error) this.emit("error", error); //emit an error because stream is destory.
