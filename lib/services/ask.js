...
class Ask extends Throught {
    ...
    constructor(request, $qwebs) {
        ...
        this.on('newListener', (event, listener) => {
            if (["data", "file", "headers"].some(e => e == event)) {
                process.nextTick(() => {
                    this.tryFlowing();
                })
            }
    ...
    async before() {
        if (/^httpAuth/.test(this.methodName)) {
            const { qwebs } = this;
            const auth = await qwebs.resolve("$auth");
            await this.auth.authenticate(ask);
        }
    }
    ...
    pipe(destination) {
        ...
        destination.emit("ask", this);
        this.tryFlowing(); //start reading request stream
        return destination;
    }
