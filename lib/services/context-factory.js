...
class ContextFactory {
    ...
    async getAsk(request, headers) {
        ...
        //     return new Ask(stream, { objectMode: true });
        // }
    ...
    async getStream(ask, reply) {
        if (!ask) throw new UndefinedError("request");
        if (!reply) throw new UndefinedError("headers");
        return new AskReply(ask, reply);
    }
}
