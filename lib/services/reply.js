...
class Reply extends Throught {
    ...
    constructor(ask, response) {
        ...
        this.mode = null;             /* object|array => use to configure the stringifier. */
    ...
    async pipeEnd(firstchunk) {
        ...
        if (chunk && this.first) {
            if (typeof chunk == "object")
                this.mode = Array.isArray(chunk) ? "array" : "object";
        }
    ...
    async jsonStrategy(stream, firstchunk) {
        ...
        const isArray = this.mode === "object" ? false : true;
