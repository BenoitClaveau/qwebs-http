...
class ResponseService {
    ...
    _envelope(responsePromise, data) {
        try {
            ...
            if (!data.request) throw new Error("Request is null.", data: data);
            ...
            switch (this.method) {
                ...
                case "PATCH":
                case "DELETE":
                    if (!data.content && !data.stream) throw new Error("Content is null.", data: data);
                    break;
                case "HEAD":
                ...
                if (data.headers["Content-Type"].match(/\bapplication\/json\b/)) {
                    if (data.content && typeof data.content !== "string") {
                        try {
                            ...
                            throw new Error("Failed to stringify content", data: data, stack: error.stack);
                        }
                    }
                    else if (data.stream) {
            ...
            try {
                if (data.contentGzip) { //assets already gzipped
                    ...
                    zlib.gzip(data.content, (error, buffer) => {
                        if (error) reject(new Error(error.message, stack: error.stack }));
    ...
    send(response, data) {
        return new Promise((resolve, reject) => {
            ...
            if (!(error instanceof DataError)) throw new Error(statusCode: 500, request: data.request, message: error.message, stack: error.stack);
